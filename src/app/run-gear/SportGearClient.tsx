...
export default function SportGearClient({ variant }: { variant: "running" | "walking" }) {
  const isWalking = variant === "walking";
  const noun = isWalking ? "walk" : "run";
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">
            {isWalking ? "Walking" : "Running"} Gear
          </h1>
          <p className="mt-1 text-sm text-gray-400 max-w-xl">
            Dress right for your {noun}. The avatar updates to the exact layers and accessories for
            current conditions where you {noun}.
          </p>
